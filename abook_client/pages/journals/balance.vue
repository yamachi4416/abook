...
const fetchData = async (store, to) => {
  ...
  const yms = [...Array(6).keys()].map(n =>
    datetime(to).subtract(n, 'months')
  ).map(date => ({
    ...
    ...d,
    key: datetime(d.from).format('YYYYMMDD') + datetime(d.to).format('YYYYMMDD')
  }))
  ...
  const [accounts, balance] = await Promise.all([
    ...
    store.dispatch('journals/getBalance', {
      ...
      periods: yms.slice(0, yms.length - 1).map(d => datetime(d.from).format('YYYYMMDD'))
    })])
    ...
    balance: balance.map((d) => {
      ...
      return {
        ...
        amount: [1, 4].includes(acc.financeDiv)
          ? d.creditAmount - d.debitAmount : d.debitAmount - d.creditAmount
      }
    }),
    yms
  }
  ...
  methods: {
    ...
    search ({ accountId, financeDiv, from, to, title }) {
      ...
      this.$router.push({
        ...
        query: {
          accrualDateStart: from ||
            datetime(yms[yms.length - 1].from).format('YYYY-MM-DD'),
          accrualDateEnd: to ||
            datetime(yms[0].to).format('YYYY-MM-DD'),
          title,
          accountId,
          financeDiv
        }
